...
void *mother(void *args){
        ...
        //Wakes up child 
        for(int i = 1; i < 13; i++){
            printf("Mother wakes child #%d up\n", i);
        }
        //Feed Breakfast
        for(int i = 1; i < 13; i++){
            printf("Mother feeds Breakfast to child #%d\n", i);
        }
        //Send to school
        for(int i = 1; i < 13; i++){
            printf("Mother sends child #%d to school\n", i);
        }
        // Give dinner
        for(int i = 1; i < 13; i++){
            printf("Mother feeds dinner to child #%d\n", i);
        }
        //Give bath
        for(int i = 1; i < 13; i++){
            printf("Mother gives a bath to child #%d\n", i);
...
void *father(void *args){
        ...
        for(int i = 1; i < 13; i++){
            ...
            sem_wait(&fatherSem);
            usleep(100);
            printf("Father reads a book to child #%d\n", i);
        }
        // Tuck child into bed
        for(int i = 1; i < 13; i++){
            printf("Father tucks child #%d into bed\n" ,i);
        }
